...
public interface WorkItemRepository extends CrudRepository<WorkItem, Long> {
    ...
    List<WorkItem> findByIsTemplateUserCanView(@Param("isTemplate") boolean isTemplate, @Param("user") User user);
    ...
    @Query("SELECT w FROM WorkItem w " +
            ...
            "OR (type(ha) = User AND ha = :user) OR (type(ce) = UserGroup AND (:user MEMBER ce.users OR :user = ce.creator)) " +
            "OR (type(ha) = UserGroup AND (:user MEMBER ha.users OR :user = ha.creator))")
    List<WorkItem> findAllCanUserView(@Param("user") User user);

    @Query("SELECT w FROM WorkItem w " +
            "left JOIN w.canEdit ce  " +
            "WHERE :user = w.owner OR (type(ce) = User AND ce = :user) " +
            "OR (type(ce) = UserGroup AND (:user MEMBER ce.users OR :user = ce.creator)) ")
    List<WorkItem> findAllCanUserEdit(@Param("user") User user);
    ...
    @Query("SELECT w FROM WorkItem w " +
            ...
            "OR (type(ce) = UserGroup AND (:user MEMBER ce.users OR :user = ce.creator))) ")
    List<WorkItem> findUsableInTaskForUserCanEdit(@Param("id") Long id, @Param("user") User user);
    ...
    @Query("SELECT w FROM WorkItem w " +
            ...
            "AND (:user = w.owner OR (type(ce) = User AND ce = :user) " +
            "OR (type(ce) = UserGroup AND (:user MEMBER ce.users OR :user = ce.creator))) ")
    List<WorkItem> findUsableInProcessForUserCanEdit(@Param("id") Long id, @Param("user") User user);
}
